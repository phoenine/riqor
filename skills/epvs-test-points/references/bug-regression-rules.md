...
- ePVS Project Profile `repositories.product` 映射出的本地 dev repo。
- 相关历史缺陷、知识库、已有手工/自动化覆盖摘要（仅用于判断风险背景；
  具体用例/自动化匹配交给 `epvs-test-cases` / `epvs-automation`）。
...
6. Handoff：把 Test Points / Coverage Gaps 交给 `epvs-test-cases` 做 Coverage Match（Existing TC / Automation 匹配）和后续 Decision。

`epvs-test-points` 不匹配现有手工用例或自动化覆盖；它只定义和审查“应该覆盖什么”。

## 产物路径

- `bug_surface: frontend` → 所有 bug 回归 Markdown 写入 `outputs/v1/` 或
  `outputs/v2/`（与 `product_line` 一致）。
- `bug_surface: backend` 或 `other` → 写入 `outputs/shared/`（如
  `outputs/shared/regression/`、`outputs/shared/test-cases/`）。
- 禁止写入 `runs/<run-id>/`。
