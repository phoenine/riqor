...
- Test Points and Coverage Gaps from `test-analysis`.
- Requirement, risk, impact, bug, or acceptance artifacts.
- Existing manual cases.
- Automation inventory from Project Profile automation repositories, when the phase includes
  automation matching.
...
coverage authority. `test-case-design` must not silently bypass a required Test
Analysis phase.
