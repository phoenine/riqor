...
上传测试用例时必须按 `zentao-cli` 的 API schema 组织步骤字段。已确认的
schema 来源：

```text
/Users/phoenine/.volta/tools/image/packages/zentao-cli/lib/node_modules/zentao-cli/dist/index.js
```
...
{
  "productID": 11,
  "title": "【v2】产线趋势默认展示最近 15 天数据",
  "precondition": "已登录 ePVS；目标产线存在 JPH 聚合数据",
  "steps": ["从区域 JPH 视图进入产线产量趋势视图"],
  "expects": ["页面默认展示最近 15 天趋势数据"],
  "stepType": ["step"],
  "pri": 1,
  "type": "feature"
}
...
```bash
zentao create testcase --product 11 --data '{"productID":11,"title":"【v2】产线趋势默认展示最近 15 天数据","precondition":"已登录 ePVS；目标产线存在 JPH 聚合数据","steps":["从区域 JPH 视图进入产线产量趋势视图"],"expects":["页面默认展示最近 15 天趋势数据"],"stepType":["step"],"pri":1,"type":"feature"}'
```
