...
`epvs-test-points` as upstream scope. Match them to existing manual cases and
...
python3 tools/run_state.py \
  --run-id <run-id> \
  --knowledge-used path=skills/epvs-test-cases/references/test-case-design-methodology.md,purpose=tp-to-tc-instantiation-rules \
  --knowledge-used path=skills/epvs-test-cases/references/coverage-review-rules.md,purpose=tp-to-tc-coverage-review \
  --knowledge-used path=skills/epvs-test-cases/references/case-writing-rules.md,purpose=case-title-and-format-rules
```
...
- Do not upload cases to ZenTao; use `epvs-zentao-sync`.
- Do not execute automation; use `epvs-automation`.
